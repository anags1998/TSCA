...
[Ik,Qk] = asignacion_simbolos(bits_cod,constelacion/Eav);
...
Pbpfm(i) = sum(bits ~= bitsr )/numero_de_bits; %tasa real de error
...
Pb_teorica(i) = fQ(sqrt(2*EbN0(i)*(10^(-0.7/10))));
end
%% figuras
figure,
semilogy(EbN0dB,Pbpfm)
hold on
semilogy(EbN0dB,Pb4psk)
hold on
semilogy(EbN0dB,Pb_teorica)
legend("Pb 8PFM TCM en funcion de EbN0","Pb 4PSK en funcion de EbN0","Pb PSK teorica")